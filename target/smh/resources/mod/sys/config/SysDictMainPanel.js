...
Ext.define('Sys.config.DictList',{
    ...
    initComponent:function(){
        ...
        this.bbar=Ext.create('Ipe.PagingToolbar',{
            ...
            items:pageCom([['dictName','名称'],['dictCode','代码']],this)
        });

        this.on('select',this.loadDictVal,this);
        this.callParent();
    },addDict:function(){
        var win=Ext.create('Sys.config.DictWindow',{parent:this,title:'新增',oper:'add'});
        win.show();
    },editDict:function(){
        var record=this.getSelectionModel().getSelection();
        if(record && record.length>0){
            var win=Ext.create('Sys.config.DictWindow',{parent:this,title:'修改',oper:'edit'});
            win.show();
...
Ext.define('Sys.config.DictValList',{
    ...
    initComponent:function(){
        ...
        if(record && record.length>0){
            var win=Ext.create('Sys.config.DictValWindow',{parent:this,title:'新增',oper:'add'});
            win.show();
            win.setDictId(record[0].data.id);
        }else{
        ...
        var record=this.getSelectionModel().getSelection();
        if(record && record.length>0){
            var win=Ext.create('Sys.config.DictValWindow',{parent:this,title:'修改',oper:'edit'});
            win.show();
            win.setData(record[0]);
        }else{
            Ext.Msg.alert('提示','请选择要编辑的记录!');
...
Ext.define('Sys.config.DictValWindow',{
    ...
    initComponent:function(){
        ...
        this.form.getForm().findField('dictId').setValue(dictId);
    },setData:function(record){
        this.form.loadRecord(record);
    },saveData:function(){
        var me=this;
...
Ext.define('Sys.config.SysDictMainPanel',{
    ...
    initComponent:function(){
        ...
        this.dictValList=Ext.create('Sys.config.DictValList',{parent:this,region:'east',width:450,split:true});
